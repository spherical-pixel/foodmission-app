...
{
    [Preserve]
    public class ChallengesScreen : NavigationScreenBase<ChallengesViewModel>
    {
        ...
        public ChallengesScreen()
        {
            InitializeComponent(App.current.services
                ...
                .Get(TemplateAddresses.ChallengesScreen));
        ...
        {
            if (_btnLevelAll != null) _btnLevelAll.clicked += () => _viewModel?.SetLevelFilter(ChallengeFilterLevel.All);
            if (_btnLevelBeginner != null) _btnLevelBeginner.clicked += () => _viewModel?.SetLevelFilter(ChallengeLevel.Beginner);
            if (_btnLevelIntermediate != null) _btnLevelIntermediate.clicked += () => _viewModel?.SetLevelFilter(ChallengeLevel.Intermediate);
            if (_btnLevelAdvanced != null) _btnLevelAdvanced.clicked += () => _viewModel?.SetLevelFilter(ChallengeLevel.Advanced);

            if (_btnStatusAll != null) _btnStatusAll.clicked += () => _viewModel?.SetStatusFilter(ChallengeFilterStatus.All);
            if (_btnStatusPending != null) _btnStatusPending.clicked += () => _viewModel?.SetStatusFilter(ChallengeFilterStatus.Pending);
            if (_btnStatusCompleted != null) _btnStatusCompleted.clicked += () => _viewModel?.SetStatusFilter(ChallengeFilterStatus.Completed);
        }
        ...
        private void UpdateFilterStates()
        {
            ...
            {
                ChallengeFilterStatus.Pending => 1,
                ChallengeFilterStatus.Completed => 2,
                _ => 0
            };
        ...
        private void RebuildHierarchy()
        {
            ...
            {
                if (group == null || group.Challenges == null || group.Challenges.Count == 0)
                    continue;
                ...
                if (group.Challenges != null)
                {
                    foreach (var cItem in group.Challenges)
                    {
                        if (cItem?.Challenge == null) continue;

                        var challengeCard = new FMItemChallenge();
                        challengeCard.Text = !string.IsNullOrEmpty(cItem.Challenge.title)
                            ? cItem.Challenge.title
