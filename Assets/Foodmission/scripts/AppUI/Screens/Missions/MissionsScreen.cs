...
{
    [Preserve]
    public class MissionsScreen : NavigationScreenBase<MissionsViewModel>
    {
        ...
        public MissionsScreen()
        {
            InitializeComponent(App.current.services
                ...
                .Get(TemplateAddresses.MissionsScreen));
        ...
        {
            if (_btnLevelAll != null) _btnLevelAll.clicked += () => _viewModel?.SetLevelFilter(MissionFilterLevel.All);
            if (_btnLevelBeginner != null) _btnLevelBeginner.clicked += () => _viewModel?.SetLevelFilter(MissionLevel.Beginner);
            if (_btnLevelIntermediate != null) _btnLevelIntermediate.clicked += () => _viewModel?.SetLevelFilter(MissionLevel.Intermediate);
            if (_btnLevelAdvanced != null) _btnLevelAdvanced.clicked += () => _viewModel?.SetLevelFilter(MissionLevel.Advanced);

            if (_btnStatusAll != null) _btnStatusAll.clicked += () => _viewModel?.SetStatusFilter(MissionFilterStatus.All);
            if (_btnStatusPending != null) _btnStatusPending.clicked += () => _viewModel?.SetStatusFilter(MissionFilterStatus.Pending);
            if (_btnStatusCompleted != null) _btnStatusCompleted.clicked += () => _viewModel?.SetStatusFilter(MissionFilterStatus.Completed);
        }
        ...
        private void UpdateFilterStates()
        {
            ...
            {
                MissionLevel.Beginner => 1,
                MissionLevel.Intermediate => 2,
                MissionLevel.Advanced => 3,
                _ => 0
            };
            ...
            {
                MissionFilterStatus.Pending => 1,
                MissionFilterStatus.Completed => 2,
                _ => 0
            };
        ...
        private void RebuildHierarchy()
        {
            ...
            {
                if (group == null || group.Missions == null || group.Missions.Count == 0)
                    continue;
                ...
                if (group.Missions != null)
                {
                    foreach (var mItem in group.Missions)
                    {
                        if (mItem?.Mission == null) continue;

                        var missionCard = new FMItemMission();
                        missionCard.Text = !string.IsNullOrEmpty(mItem.Mission.title)
                            ? mItem.Mission.title
                            : (mItem.Mission.code ?? "");
