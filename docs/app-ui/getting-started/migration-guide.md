...
The `SplitView` component has been completely refactored and does not inherit from [TwoPaneSplitView](xref:UIElements.TwoPaneSplitView) anymore. It's a brand new implementation that gives you more flexibility and control over the split view. This component can now contain any number of [Pane](../api/Unity.AppUI.UI.Pane.html) elements as children.
...
Most of the `Dropzone` logic has been moved into a new `DropzoneController` class. The `Dropzone` class now only handles the visual representation of the dropzone, but you have access to the `DropzoneController` instance to handle the drop logic. Also, instead of trying to get droppables from paths or Unity objects, the controller offers a more generic `acceptDrag` delegate to determine if the dragged object should be accepted. The `dragStarted` event has been removed since the main goal of the Dropzone is to handle the drop event.
...
Please read the [State Management](../references/architecture/state-management.md) guide for more information.

### Create a store and slices

When the [Store<TState>](../api/Unity.AppUI.Redux.Store-1.html) instance is constructed and returned, it is ready to be used. You cannot add slices to the store after it has been created.
...
When subscribing to store changes, instead of returning a function to unsubscribe, an [IDisposableSubscription](../api/Unity.AppUI.Redux.IDisposableSubscription.html) instance is returned. You can check its validity and dispose of it.
...
In the case you are creating your own implementation of `INavigationScreen`, make sure to create also an implementation of `NavDestinationTemplate` that contains all the logic you need to set up your screen creation. Implementing your own `INavigationScreen` means you have to call the `SetupXXX` methods of `INavVisualController` yourself during `OnEnter` callback. See [Navigation](../references/navigation.md) for more information.
